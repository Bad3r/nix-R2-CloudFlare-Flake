...
The `r2-explorer/` directory is a split API/web Worker subflake with
contract-first APIs, an Astro operator UI, and local test coverage.
...
- Zod request/response schemas in `r2-explorer/src/schemas.ts` used across API
  handlers.
- Astro + Preact operator UI in `r2-explorer/web/` calling `/api/v2/*` on the
  same origin.
...
- Share lifecycle:
  - create/list/revoke on `/api/v2/share/*`
...
- Runtime capability endpoint: `GET /api/v2/session/info`.
...
| Route                           | Purpose                                          |
| ------------------------------- | ------------------------------------------------ |
| `GET /api/v2/list`              | List objects/prefixes                            |
| `GET /api/v2/meta`              | Object metadata                                  |
| `GET /api/v2/download`          | Download with attachment disposition             |
| `GET /api/v2/preview`           | Inline/attachment preview by content type        |
| `POST /api/v2/upload/init`      | Start multipart upload session                   |
| `POST /api/v2/upload/sign-part` | Sign a single multipart part upload URL          |
| `POST /api/v2/upload/complete`  | Complete multipart upload                        |
| `POST /api/v2/upload/abort`     | Abort multipart upload                           |
| `POST /api/v2/object/delete`    | Soft-delete object to `.trash/`                  |
| `POST /api/v2/object/move`      | Move/rename object                               |
| `POST /api/v2/share/create`     | Create share token (Access or HMAC admin auth)   |
| `POST /api/v2/share/revoke`     | Revoke share token (Access or HMAC admin auth)   |
| `GET /api/v2/share/list`        | List share records for key (Access or HMAC auth) |
| `GET /api/v2/session/info`      | Runtime capabilities and effective limits        |
| `GET /share/<token>`            | Public tokenized object access                   |

### Secure Multipart Contract

Control-plane (`/api/v2/upload/*`) and data-plane (direct R2 signed `PUT`) are
strictly separated.

Control-plane contract:

- `POST /api/v2/upload/init` request includes `filename`, `declaredSize`,
  optional `prefix`, optional `contentType`, optional `sha256`.
- `POST /api/v2/upload/init` response includes `sessionId`, `uploadId`,
  `objectKey`, `expiresAt`, `partSizeBytes`, `maxParts`, `signPartTtlSec`,
  `allowedMime`, `allowedExt`.
- `POST /api/v2/upload/sign-part` request includes `sessionId`, `uploadId`,
  `partNumber`, `contentLength`, optional `contentMd5`.
- `POST /api/v2/upload/complete` request includes `sessionId`, `uploadId`,
  `parts`, optional `finalSize`.
- `POST /api/v2/upload/abort` request includes `sessionId`, `uploadId`.
...
`/api/v2/*` routes require Cloudflare Access identity. CLI-driven Worker share
...
| `r2-explorer/src/version.ts`                     | Worker version constant exposed by `/api/v2/session/info` |
| `r2-explorer/web/src/pages/index.astro`          | Astro entry page for operator console                     |
| `r2-explorer/web/src/components/OpsExplorer.tsx` | Preact operations dashboard                               |
...
curl -s https://files.unsigned.sh/api/v2/session/info | jq .
```
...
curl -sS -X POST https://files.unsigned.sh/api/v2/upload/init \
