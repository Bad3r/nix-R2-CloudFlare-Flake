...
| `services.r2-sync.mounts`                                 | attrset of submodules                                        | `{}`          | yes (must contain at least one mount) | One mount profile per attr key. Each key must match `[A-Za-z0-9_.-]+` (used verbatim in systemd unit names).          |
...
fire in lockstep against the R2 API.
